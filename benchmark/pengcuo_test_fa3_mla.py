...
for attn_variant in attn_variants[3:4]:
    ...
    print(f"should_run_flashmla = {should_run_flashmla}")
    ...
    for seqlen in [(int)(s * 1024) for s in [0.5, 1, 2, 4, 8, 16, 32, 64]]:
        ...
        # print(f"Seqlen = {seqlen}, FA3 time{'' if not use_bench_cudagraph else ' w CUDA Graph'}: {t0 * 1e3:.1f} us, {mem_io * 1e-9 / (t0 * 1e-3):.0f} GB/s, {flops * 1e-12 / (t0 * 1e-3):.0f} TFLOPS/s, ContextLen : {seqlen}")
        ...
        if should_run_flashmla:
            ...
            t = t1
            # print(
            #         f"flashmla, {b=}, {s_q=}, {h_q=}, {h_kv=}, {d=}, {dv=}, {causal=},  {t:.6f} , {FLOPS / 10 ** 9 / t:.0f} , {FLOPS}, {bytes / 10 ** 6 / t:.0f},  {bytes}, {mean_sk=}, "
            # )
